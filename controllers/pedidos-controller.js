...
exports.postPedidos = async (req, res, next) => {
    try {
        const queryProduct = "SELECT * FROM produtos WHERE id_produto = ?";
        const resultProduct = await mysql.execute(queryProduct, [req.body.id_produto]);
        if(resultProduct.length == 0){
            return res.status(404).send({
                mensagem: 'Produto não encontrado'
            })
        }
        const queryOrder = "INSERT INTO pedidos (id_produto, quantidade) VALUES (?,?)";
        ...
        const response = {
            ...
            pedidoCriado: {
                id_pedido: resultOrder.id_pedido,
